...
| `github` | GitHub Issue + Projects カード + DoD フォローコメント | opt-in | local からの同期先 |

既定は `local`。local では Story = `<epic-dir>/<slug>.md`（OKF concept）を生成するのが主フローである。Epic 以上の階層は常に markdown ファイルで管理する（変更なし）。

> **github プロバイダについて**: `github` プロバイダのアダプタ（`references/backends/github.md`）は実装済みであり、`--github` 指定時は Issue 作成・Projects 連携・フィールド設定・DoD フォローコメントを実行できる。既定は `local` であり、github は opt-in（local からの同期先）という位置づけである。local で設計した Story を後から github へ上げる運用には `sync-stories` を使う。見積もり・リスクへの遷移を担う `estimate-points` / `identify-risks`、品質ゲートの `review-stories`、hook 連携（`notify-draft-added.sh` 等）もすべて現行スキル / hook として提供済みであり、本スキルからは現行のものとして参照してよい（ただし後続スキルの直接呼び出しはせず、遷移は hook が担う。責務分離は維持する）。
...
| Points / Risk Score | `set_field` で初期値 0 をセット（local では frontmatter）。実値は `estimate-points` / `identify-risks` が後段で更新 |
...
| 項目 | 値 |
|------|----|
| Story 項目 | `create_item` で GitHub Issue を作成（github アダプタ） |
| ラベル | `story` + `epic:<epic-id>` + `draft` を `add_label` で付与 |
| 階層フィールド | `set_field` で Projects のフィールドに反映（github アダプタ） |
| DoD | `add_comment` でフォローコメント投稿（github アダプタ） |
...
- `tracker.provider` で出力先を設定（`local` 既定 / `github` は opt-in）
...
- 未設定の操作は graceful degradation（アダプタがスキップして警告。`tracker-contract.md` §5）
...
- **github**: `gh` 認証・`project` スコープ・`tracker.github.*` の確認。未設定操作は graceful degradation（`references/backends/github.md`）
...
雛形の「見積もり詳細」セクションは **初期値（Points=0, O=M=P=0）で出力**。実値は `estimate-points` が後段で更新する。登録時は `set_field(item_ref, "points", 0)` で初期化する。

#### 5-7. リスクと対応策（プレースホルダー）

「リスクと対応策」セクションも空のテーブルで出力。`set_field(item_ref, "risk_score", 0)` で初期化する。`identify-risks` が後段で埋める。設計の過程で明らかに重大なリスクが見えた場合のみ、その場で 1 行入れておく。
...
- **本文**:
  ...
  - 見積もり詳細・リスクと対応策はテーブル雛形のまま（`{{POINTS}}` = 0 / `{{O}}` = 0 等）残す。`estimate-points` / `identify-risks` が後段で埋める
...
1. **`ensure_label`**: 付与するラベルの存在を保証する。
   ...
   - その他の追加ラベル（hotfix 等）
   - local アダプタでは no-op（ラベルは frontmatter の文字列にすぎない）。github アダプタでは未存在時に `AskUserQuestion` で「作成する / スキップ」を尋ねてから作成する（勝手にラベルを増やさない）
...
4. **`set_field`**: 階層フィールドと初期メトリクスをセットする（`tracker-contract.md` §3 の正規語彙）:
   ...
   - local では frontmatter の対応キーに保存。github では Projects フィールドに反映。未設定のフィールドはアダプタがスキップし、結果サマリーに表示する（graceful degradation）

> **`draft` ラベルの設計意図**:
> - 本スキルが登録する Story は、登録時点では見積もり・リスクが未確定であるのが普通である
> - `draft` を付けることで「これは下書きであり、レビューを経て初めて計画に組み込める」状態を視覚化する
> - 昇格（`draft` 削除）は **専用の `review-stories` スキル** で行う。本スキルは昇格に踏み込まない
> - レビュー前の Story を `list_items({label: "draft"})` で除外・抽出できる

> **見積もり・リスク・レビューへの遷移**: `estimate-points` / `identify-risks` / `review-stories` はいずれも現行スキルとして提供済みである。本スキルは Story 登録で完結し、後続スキルを直接呼び出さない（責務分離）。github プロバイダでは、プラグイン同梱の `hooks/notify-draft-added.sh` が `draft` 付与を検知し、`estimate-points` への遷移を Claude に促す設計である。local プロバイダでは Issue ラベル付与イベントが発生しないため hook は発火せず、ユーザーが必要に応じて `estimate-points` 等を明示的に起動する。
...
- **github**: Issue にフォローコメントとして投稿する（`references/backends/github.md`）
...
4. github で `resource`（同期済み Issue URL）が既にセットされている場合、ユーザーに「合わせて同期先も更新するか」を確認する

> frontmatter の `points` / `estimation.*` / `risks` / `risk_score` は本スキルでは **触らない**（`estimate-points` / `identify-risks` の責務）。`tracker_ref` / `github_issue` / `uploaded_at` も本スキルでは触らない（`sync-stories` の責務）。
...
local モードの場合、サマリーの末尾に **「次のアクション候補」** として以下を案内する:

- `estimate-points`: ローカル Story の見積もりを行う
- `identify-risks`: ローカル Story のリスクを識別する
- `sync-stories`: ローカル Story を github トラッカーへアップロードする
- `review-stories`: 品質ゲートを通して `draft` を外す
- `build-bundle`: OKF バンドルの `index.md` / `log.md` を補完・検証する

サマリーを出した後、本スキル単体は終了する。後続スキルの起動は **hooks のイベントルーターに集約** する設計である（責務分離）。本スキルから後続スキルを直接呼び出さない。github プロバイダでは `hooks/notify-draft-added.sh` が `draft` 付与を検知して `estimate-points` への遷移を促す。local プロバイダでは hook が発火しないため、ユーザーが必要に応じて各スキルを起動する。
...
- **provider 別の出力先**:
  ...
  - github: GitHub Issue + Projects 側のみ変更
...
- **登録直後の Story は必ず `draft` 状態**。品質ゲートは `review-stories` に分離する。本スキルでは昇格判定に踏み込まない
...
- 見積もり値（Points / O / M / P）とリスクスコアは登録時点では埋めない。`set_field` で 0 に初期化し、`estimate-points` / `identify-risks` が後段で埋める
...
- **github の Projects フィールド option 追加に関するガードレール**（Single Select option の CLI / GraphQL 書き換え禁止）は、`provider == github` のとき hook（`guard-project-field-mutation.sh`）が作動してハードブロックする。local 運用では Projects 概念がないため該当しない
...
- **frontmatter が常に source of truth**。本文の表（見積もり詳細・リスク表）は frontmatter と連動させる（`estimate-points` / `identify-risks` が両方を同時更新）
- **DoD は本文末尾の「Definition of Done」セクションに展開**（`add_comment` の local 実装）。github へ同期する際は、本セクションの内容を分離してフォローコメントとして投稿する（`sync-stories` の責務）
